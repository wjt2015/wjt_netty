 ...
 * implementation.  {@link MyChunkedWriteHandler} manages such complicated states
 * so that you can send a large data stream without difficulties.
 * <p>
 * To use {@link MyChunkedWriteHandler} in your application, you have to insert
 * a new {@link MyChunkedWriteHandler} instance:
 * <pre>
 * {@link ChannelPipeline} p = ...;
 * p.addLast("streamer", <b>new {@link MyChunkedWriteHandler}()</b>);
 * p.addLast("handler", new MyHandler());
 * </pre>
 * Once inserted, you can write a {@link ChunkedInput} so that the
 * {@link MyChunkedWriteHandler} can pick it up and fetch the content of the
 ...
 */
@Slf4j
public class MyChunkedWriteHandler extends ChannelDuplexHandler {

    private static final InternalLogger logger =
            InternalLoggerFactory.getInstance(MyChunkedWriteHandler.class);

    private final Queue<PendingWrite> queue = new ArrayDeque<PendingWrite>();
    private volatile ChannelHandlerContext ctx;
    private PendingWrite currentWrite;

    public MyChunkedWriteHandler() {
    }

    /**
     * @deprecated use {@link #MyChunkedWriteHandler()}
     */
    @Deprecated
    public MyChunkedWriteHandler(int maxPendingWrites) {
